...
const recordTableName: string = "ethereum.hourlyBasicNetStats";

// socket.io-clientの定義
const socketClientName: string = "hourlyBasicNetStatsRecorder";
...
socketClient.on("newBlockDataRecorded", async (blockNumberWithTimestamp: blockNumberWithTimestamp) => {
    ...
    if (!isRecording) {
        ...
        let [endTimeOfHourlyBasicNetStats] = await pool.query<RowDataPacket[any]>(`SELECT endTimeUnix
                                                                                     FROM ${recordTableName}
                                                                                     ORDER BY endTimeUnix DESC LIMIT 1`);

        if (endTimeOfHourlyBasicNetStats.length === 0) {
            // データベースが空の場合には0のタイムスタンプを設定
            endTimeOfHourlyBasicNetStats[0] = {endTimeUnix: 0};
        }

        if (blockNumberWithTimestamp.timestamp) {
            // タイムスタンプの値がTruthyなら集計期間の開始時間と終了時間を示すtimeRangeArray配列を生成
            let timeRangeArray: timeRangeArray = timeRangeArrayMaker(endTimeOfHourlyBasicNetStats[0].endTimeUnix, blockNumberWithTimestamp.timestamp, DURATION);
